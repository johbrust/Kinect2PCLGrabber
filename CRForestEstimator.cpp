...
void CRForestEstimator::loadConfig(const char* filename) {
	...
	if (in.is_open()) {
		...
		cout << "Forest Configuration of loaded." << endl;
	}
	else {
...
Eigen::Affine3f CRForestEstimator::GetTransformation(float x, float y, float z, float roll, float pitch, float yaw) {
	Affine3f rotX = Affine3f(AngleAxisf(rad(roll), Vector3f(1, 0, 0)));
	Affine3f rotY = Affine3f(AngleAxisf(rad(pitch), Vector3f(0, 1, 0)));
	Affine3f rotZ = Affine3f(AngleAxisf(rad(yaw), Vector3f(0, 0, 1)));
	Affine3f Rotation = rotZ * rotY * rotX;
	Affine3f Translation(Translation3f(Vector3f(x, y, z)));
	//cout << "Translation: " << endl << Translation.matrix() << endl;
